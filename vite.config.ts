...
export default defineConfig({
  plugins: [
    react(),
    // copy the dv01-web-components.d.ts file to the dist folder
    viteStaticCopy({
      targets: [
        { src: 'src/dv01-web-components.d.ts', dest: '' },
      ],
    }),
  ...
  build: {
    lib: {
      //all custom elements defined in this file will be bundled and defined into dv01-web-components.[es|umd].js
      entry: 'src/defineCustomElements.ts',
      name: 'CustomElements',
      fileName: (format) => `dv01-web-components.${format}.js`,
      formats: ['es', 'umd'],
    },
    // not externalizing to encapsulate packages needed for the web components
    // rollupOptions: {
    //   // external option - rely on consumer to have react and react-dom installed - but this results in error when trying to use the umd file
